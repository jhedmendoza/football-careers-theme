...
<li <?php job_listing_class("job-listing grid"); ?>
...
data-image="<?php echo (get_the_company_logo( $logo_id, 'thumbnail' )) ?  get_the_company_logo( $logo_id, 'thumbnail' ) : apply_filters( 'job_manager_default_company_logo', JOB_MANAGER_PLUGIN_URL . '/assets/images/company.png' ); ?>"
...
data-job_id="<?php echo esc_attr($post->ID); ?>"
			...
			<?php do_action( 'workscout_job_listing_meta_start' ); ?>
				...
				if (in_array("date", $job_meta)) {
					if(workscout_newly_posted()) {
						...
						<li  class="job-listing-footer-date"><div class="listing-date"> <?php the_job_publish_date() ?></div></li>
					<?php	}
				} ?>
				<?php if (in_array("deadline", $job_meta)) { ?>
					...
					<?php if (in_array("expires", $job_meta)) { ?>
						<li class="job-listing-footer-expires"><div class="listing-date">
...
<?php if($types_below): ?>class="types_below"<?php endif; ?>
>
	<section <?php job_listing_class($position); ?> data-longitude="<?php echo esc_attr( $post->geolocation_long ); ?>" data-latitude="<?php echo esc_attr( $post->geolocation_lat ); ?>">
